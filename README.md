...
> - 有色矿山机械AR+AI视觉质检方案_优化版.pptx —— 商务/分层方案书（**本地文件，不入库**，见下）
> - [铸件表面缺陷视觉检测可行性结论.md](doc/铸件表面缺陷视觉检测可行性结论.md) —— 针对客户实拍样张（`AI识别.pdf`）的落地可行性评估
> - [成像POC器材清单与测试步骤.md](doc/成像POC器材清单与测试步骤.md) —— 落地前成像验证：买什么、怎么做打光对照实验
>
> **`doc/` 下的 pdf / pptx / docx 原件不入库**（2026-09-10 起）：它们无法 delta 压缩，每改一版就永久
> 堆一个 blob。结论与依据一律以同名 md 入库，原件留在本地 `doc/` 与共享盘。
