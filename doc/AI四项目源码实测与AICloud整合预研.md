...
> ---
> ## ★ 2026-09-16 订正指针（原文不动，请连同本条一起读）
>
> 本文对 v4 / v5 的刻画**有两处不准**，2026-09-16 再次只读核查时查出。**原文保留不改**
> （写错的东西要承认，不悄悄改掉），订正与依据见
> [`AIIntegration/README.md` §18](../AIIntegration/README.md)：
>
> 1. **"v4 高频振动 / v5 低频振动"这个划分不成立** —— 两者的阶次诊断
>    （FFT / 包络 / `amp_1x`~`amp_5x` / 规则卡 / KNN / MLP）是**同一份代码**，
>    12 个 `.py` + 5 个规则卡 **sha256 逐位相同**。真正的区别是：
>    **v4 = 离线波形分析 + 四个机型的专用诊断（阶次振动 / 压装 / 伺服 / 机械臂）；
>    v5 = 同一套阶次诊断 + 接入实时库的工作台闭环。**
> 2. **"两边的 `order_diagnosis` 已经不一致"—— 今天实测不成立**（17 个文件全同）。
>    「同一套算法在两处并存会漂」这个**风险判断仍然成立，只是它还没有发生**。
>
> 另补一件本文没记的：按"不重复"去重，四个项目里共有 **12 个算法模块**
> （本文按"四个项目 = 四类能力"记账，粒度太粗，而我方域是按模块划分的）。
> 清单见 `README §18.6`。
>
> ---
